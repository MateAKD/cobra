...
# ========================================
# HARDENING DE SEGURIDAD - VPS COBRA 2.0 (SAFE MODE)
# ========================================
# Protección contra CVE-2025-55182 y malware
# Uso: sudo bash harden-safe.sh
...
echo -e "${BLUE}🔒 ==========================================${NC}"
echo -e "${BLUE}🔒 HARDENING DE SEGURIDAD - COBRA 2.0 (SAFE)${NC}"
echo -e "${BLUE}🔒 ==========================================${NC}"
echo ""
...
# ========================================
# 5. HARDENING SSH (SAFE MODE)
# ========================================
log "[5/12] Aplicando hardening a SSH (SAFE MODE: Password Allowed)..."

# Backup de configuración original
cp /etc/ssh/sshd_config /etc/ssh/sshd_config.backup

# Configuraciones de seguridad
# NOTA: PasswordAuthentication se mantiene como 'yes' por solicitud del usuario
...
log "   ✅ SSH configurado (Contraseñas habilitadas)"
...
# Instalar cron job para monitor-malware.sh (solo si está subido)
if [ -f /var/www/cobra/monitor-malware.sh ]; then
    ...
    warning "   ⚠️  monitor-malware.sh no encontrado en /var/www/cobra. Saltando cron job."
fi
...
if [ -f /etc/nginx/nginx.conf ]; then
    ...
    if ! grep -q "limit_req_zone" /etc/nginx/nginx.conf; then
        # Agregar rate limiting al bloque http - USANDO sed CON CUIDADO
        # Insertar después de 'http {'
        ...
        log "   ℹ️  Rate limiting ya estaba configurado"
    fi
fi
...
echo -e "${GREEN}================================================${NC}"
echo ""
warning "⚠️  IMPORTANTE: No cierres esta sesión hasta probar que puedes volver a entrar en otra terminal."
